...
def StartBrowser(browserChoice):
    ...
    # print(browserTitle)
    if browserTitle == "Sign-In Verification | LinkedIn":
        verificationCode = int(input('Code from Email: '))
        if not verificationCode is None:
            codeElement = browser.find_element_by_id('verification-code')
            codeElement.send_keys(verificationCode)
            codeElement.submit()
...
def LInBot(browser):
        ...
        while True:
            ...
            while True:
                ...
                with open('visitedUsers.txt', 'wb') as visitedUsersFile:
    ...
    try:
        for link in soup.find('div', {'class':'insights-browse-map'}).findAll('a', {'class':'browse-map-photo'}):
            if 'profile/view?id=' in link.get('href'):
                profileID = parse.parse_qs((parse.urlparse(link.get('href'))).query)['id'][0]
